...
// height and width so the whole reveal fits on any display, from a phone to an LED wall.
export default function LuckyWinnerReveal({ winner, isFullscreen }: Props) {
  const t = useT();
  const isVip = winner.attendee_type === 'VIP';
  const visual = isFullscreen ? 'w-[min(20rem,30vh,26vw)] h-[min(20rem,30vh,26vw)]' : 'w-[min(13rem,22vh,26vw)] h-[min(13rem,22vh,26vw)]';
  ...
  return (
    <div className="relative z-10 w-full flex flex-col items-center">
      ...
      <div className={`relative winner-reveal inline-flex items-center gap-3 font-mono font-bold uppercase text-amber-200 ${isFullscreen ? 'text-base sm:text-xl tracking-[0.3em] sm:tracking-[0.45em] mt-4 mb-6 sm:mb-8' : 'text-xs sm:text-sm tracking-[0.25em] sm:tracking-[0.35em] mt-3 mb-5'}`}>
      ...
      <div className="relative flex items-center justify-center gap-3 sm:gap-8">
          ...
          <div className={`relative ${visual}`}>
            ...
            <div className="absolute inset-[9px] rounded-full overflow-hidden bg-gradient-to-br from-indigo-600 via-purple-600 to-amber-500 flex items-center justify-center">
              {winner.profile_picture ? (
                ...
                <span className={`font-black text-on-accent drop-shadow-lg ${isFullscreen ? 'text-5xl sm:text-8xl' : 'text-4xl sm:text-5xl'}`}>{initials(winner.name)}</span>
              )}
            </div>
            {isVip && (
              <div className="absolute left-1/2 -translate-x-1/2 -top-[9%] text-amber-300 drop-shadow-[0_0_18px_rgba(251,191,36,.8)]">
                <Crown className={isFullscreen ? 'w-10 h-10 sm:w-16 sm:h-16' : 'w-8 h-8 sm:w-10 sm:h-10'} fill="currentColor" />
              </div>
            )}
            {isVip && (
        ...
        <div className="winner-reveal flex flex-col items-center gap-2 text-amber-300">
          <div className={`winner-link h-[3px] rounded-full ${isFullscreen ? 'w-12 sm:w-28' : 'w-10 sm:w-20'}`} />
          <Trophy className={`drop-shadow-[0_0_20px_rgba(251,191,36,.7)] ${isFullscreen ? 'w-9 h-9 sm:w-14 sm:h-14' : 'w-7 h-7 sm:w-9 sm:h-9'}`} />
          <div className={`winner-link h-[3px] rounded-full ${isFullscreen ? 'w-12 sm:w-28' : 'w-10 sm:w-20'}`} />
        </div>
        ...
        <div className="winner-reveal-late relative flex flex-col items-center">
          <div className={`winner-prize-frame relative ${visual} rounded-[2rem] border border-amber-300/40 bg-gradient-to-br from-amber-400/15 via-purple-500/15 to-cyan-400/10 backdrop-blur-xl overflow-hidden flex items-center justify-center`}>
            ...
            {winner.prize_image ? (
              ...
              <Gift className={`lucky-prize-float relative text-amber-200 drop-shadow-[0_0_30px_rgba(251,191,36,.7)] ${isFullscreen ? 'w-16 h-16 sm:w-32 sm:h-32' : 'w-12 h-12 sm:w-20 sm:h-20'}`} strokeWidth={1.4} />
            )}
          </div>
        </div>
      </div>
      ...
      <div key={winner.name} className={`winner-reveal-late text-center max-w-full ${isFullscreen ? 'mt-10' : 'mt-6'}`}>
        <div className={`winner-name font-black leading-tight break-words px-2 sm:px-4 ${isFullscreen ? 'text-4xl sm:text-7xl lg:text-8xl' : 'text-3xl sm:text-5xl lg:text-6xl'}`}>
          {winner.name}
        </div>
        {(winner.company || winner.position) && (
          <div className={`text-indigo-100/90 flex items-center justify-center gap-2 flex-wrap ${isFullscreen ? 'text-lg sm:text-2xl mt-3' : 'text-base sm:text-lg mt-2'}`}>
        ...
        <div className={`inline-flex flex-wrap items-center justify-center gap-x-3 gap-y-1 max-w-full rounded-2xl border border-amber-300/40 bg-gradient-to-r from-amber-500/20 via-purple-500/20 to-cyan-500/15 shadow-[0_0_40px_rgba(251,191,36,.18)] ${isFullscreen ? 'mt-7 px-5 sm:px-8 py-3 sm:py-4' : 'mt-5 px-4 sm:px-5 py-2.5'}`}>
          ...
          <span className={`font-extrabold text-white ${isFullscreen ? 'text-xl sm:text-3xl' : 'text-lg sm:text-xl'}`}>{winner.prize_name}</span>
        </div>
