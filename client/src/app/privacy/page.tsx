...
export default function PrivacyPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 pb-8">
      <Link href="/register" className="inline-flex items-center gap-2 text-sm text-slate-400 hover:text-white"><ArrowLeft className="w-4 h-4" />กลับไปหน้าลงทะเบียน</Link>
      ...
      <div className="glass-panel rounded-3xl border border-white/10 p-6 sm:p-10">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-white">นโยบายความเป็นส่วนตัว</h1>
            <p className="text-sm text-slate-400 mt-1">{owner} · ตาม พ.ร.บ. คุ้มครองข้อมูลส่วนบุคคล พ.ศ. 2562 (PDPA)</p>
          </div>
        </div>
        ...
        {paragraphs.length > 0 ? (
          ...
          <p className="text-base text-slate-400 leading-relaxed">
            ผู้จัดงานยังไม่ได้เผยแพร่นโยบายความเป็นส่วนตัวฉบับเต็ม หากมีคำถามเกี่ยวกับการเก็บและใช้ข้อมูลส่วนบุคคลของท่าน กรุณาติดต่อผู้จัดงานตามช่องทางด้านล่าง
          </p>
        )}

        {(settings.contact_email || settings.contact_phone) && (
          <div className="mt-8 pt-6 border-t border-white/10 text-sm text-slate-300 space-y-2">
            <div className="font-bold text-white">ติดต่อเรื่องข้อมูลส่วนบุคคล</div>
